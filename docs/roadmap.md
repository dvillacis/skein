...
| M8 — Distribution & DX | ⏳ partial | M8.1 CI + M8.2 wheels + M8.3 docs (scaffold + concept pages + porting + extending + examples + API ref via mkdocstrings + Read the Docs hosting config) done; comprehensive subclass docstring pass + comparison benches + numerical regression tests + stable Rust API contract pending |
