...
use skein_core::{
    ...
    design::{Augmented, DenseMatrix, DesignMatrix, MmapMatrix, SparseCSC, Standardized},
    groups::Groups,
...
fn solve_mcp_ls_path_mmap<'py>(
    ...
    // Compute glmnet scales from the file (one full pass, paid by the
    // page cache; norms are cheap to derive alongside).
    let scales_user: Option<Array1<f64>> = if standardize_x {
        let n_f = n_rows as f64;
        let mut s = Array1::<f64>::ones(n_cols);
        for j in 0..n_cols {
            // ‖X[:,j]‖² is already cached on MmapMatrix; we still need
            // x̄_j for the glmnet variance, so do one more inner-product
            // pass.
            let col = mmap.columns(&[j]);
            let mut sum = 0.0_f64;
            for &v in col.iter() {
                sum += v;
            }
            let mean = sum / n_f;
            let sq = mmap.col_sq_norm(j);
    ...
    // Build the design tower: Mmap → (optional Augmented) → (optional Standardized).
    // The match collapses to one of four shapes; each branch hands a
    // concrete `&dyn DesignMatrix` to `solve_path`.
    let p_eff = if fit_intercept { n_cols + 1 } else { n_cols };
    let (betas_aug, report) = match (fit_intercept, scales_user.as_ref()) {
        (false, None) => solve_path(&mmap, &datafit, make_pen, &path_cfg),
        (false, Some(scales)) => {
            let std_design = Standardized::new(mmap, scales.clone());
            solve_path(&std_design, &datafit, make_pen, &path_cfg)
        }
        (true, None) => {
            let aug = Augmented::new(mmap);
