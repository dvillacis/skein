...
| M12 — Hardening (robustness, test coverage, CI) | ✅ done | Penalty + datafit unit-test coverage closed; Rust integration test directory added; R-fixture gate in CI; PyO3-layer smoke job (`.github/workflows/bench-smoke.yml`); pre-flight protocol enforced via `feedback_solver_convergence_changes.md`. No new algorithmic surface. |
...
Test count at this snapshot: **343 cargo + 412 pytest, all green.**
...
- **C5 — parallel block-CD Lipschitz caveat.** Per-group Lipschitz is
  exact for serial Gauss-Seidel; Jacobi parallel mode assumes small
  `Xᵀ_g X_{g'}` cross-coupling and silently falls back to serial for
  overlapping groups. Add an explicit overlap check + warning, plus
  a fixture that exercises the fallback.

### Robustness gaps

- **R1 — unwrap clusters in path solvers.** 59 `unwrap()` / `expect()`
  hits across 16 files in `crates/skein-core/src/`. Top offenders:
  `design/mmap.rs` (10), `design/mmap_f32.rs` (10) — mostly tempfile
  test code, acceptable. Hot user paths needing a sweep:
  `solver/path.rs` (8), `solver/block_path.rs` (5),
  `standardize.rs` (7). Convert "should never fail" invariants to
  `debug_assert!` + typed errors.
...
- **R3 — solver pre-flight protocol unenforced.** CLAUDE.md mandates
  `cargo test -p skein-core solve_path_at_tol_1e_12 -- --nocapture`
  before any convergence-logic change after the gap-safe screening
  incident. Wire this into a pre-commit hook or a feature-gated
  `cargo test --features tight-tol` profile so it isn't a "remember
  to run it" rule.
- **R4 — numerical guards scattered.** `eta_clamp [±30]` and weight
  floors (`1e-6` / `1e-8`) per M3.4.1 / M3.5.1 / M3.7 are redefined
  per datafit. Centralize so a cross-cutting bump is one edit.
...
- **P2 — criterion bench tree is one file.**
  `crates/skein-core/benches/` contains only `block_cd.rs`. Missing:
  LLA outer-loop microbench, Frobenius vs operator-norm Lipschitz
  comparison, prox_newton GLM inner solver, glasso ADMM sweep. M9.2
  is open.
...
- **P4 — `skein-py/src/lib.rs` is 10,628 lines of PyO3 glue.**
  ~60 `solve_*_path` exports in one file. Slow to compile and hard
  to audit. Split by datafit family (one file per LS / GLM /
  multitask / glasso) to speed `maturin develop` iteration and lower
  the cost of new-penalty additions.
