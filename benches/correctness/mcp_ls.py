"""Cross-package agreement on MCP / Gaussian LS.

Fits the same MCP-penalised LS problem with skein, skglm, and
R/ncvreg on a shared λ-grid and γ, then reports per-λ active-set
Jaccard, sign agreement, and relative L2 distance between every pair
of coefficient paths.

MCP is nonconvex, so packages can land at different local minima even
at the same tolerance. This script reports agreement; it does not
assert it. The headline number we care about is *support agreement* —
do the three solvers identify the same nonzero features as λ shrinks?
Coefficient-magnitude disagreement at saturated λ is expected and
called out in M9.4 of the roadmap.

Run as a script:

    python benches/correctness/mcp_ls.py --size small
    python benches/correctness/mcp_ls.py --size medium --n-lambdas 50
"""
...
logger = logging.getLogger("benches.correctness.mcp_ls")
...
def main() -> int:
    ...
    out = cc.write_results("mcp_ls", payload)
